...
public class AndroidLogProcessor extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment processingEnvironment) {
        ...
        messager.printMessage(Diagnostic.Kind.NOTE, "AndroidLogProcessor init");
    }

    @Override
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        messager.printMessage(Diagnostic.Kind.NOTE, "AndroidLogProcessor process: annotations = " + set + ", env = " + roundEnvironment);
        if (!roundEnvironment.processingOver() && trees != null) {
            roundEnvironment.getRootElements().stream()
                    ...
                    .forEach(it -> ((JCTree) trees.getTree(it))
                            .accept(new DeleteLogTranslator()));
        }
        return false;
    }
